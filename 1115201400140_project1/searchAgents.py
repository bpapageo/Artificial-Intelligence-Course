...
def cornersHeuristic(state, problem):
    ...
    for x in range(len(remaining)):
        ...
        for corner in remaining[:]:
            dist = abs(start[0] - corner[0]) + abs(start[1] - corner[1]) 
            if dist < minn:
                minn = dist
                nextcorner = corner

        summ += minn
        remaining.remove(nextcorner)
        start = nextcorner
...
def foodHeuristic(state, problem):
    ...
    nextf = ListFood[0]
    maxx = util.manhattanDistance(position, nextf)
    ...
    return util.manhattanDistance(position, nextf)
